...
function CommandAddItem({ query, onCreate }: { query: string; onCreate: () => void }) {
  return (
    <div
      ...
      className={cn(
        'flex w-full cursor-pointer text-sm px-2 py-1.5 rounded-sm items-center focus:outline-none',
...
export function Combobox({ options, selected, className, placeholder, disalbed, onChange, onCreate }: ComboboxProps) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          aria-expanded={open}
          className={cn(
            'w-full font-normal border-input focus:bg-accent focus:text-accent-foreground  focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2',
            className
          )}
        >
          {selected && selected.length > 0 ? (
            <div className="mr-auto truncate">{options.find((item) => item.value === selected)?.label}</div>
          ) : (
            <div className="mr-auto text-slate-600">{placeholder ?? 'Select'}</div>
          )}
      ...
      <PopoverContent className="w-full min-w-[300px] p-0">
        ...
        >
          <CommandInput
            placeholder="Recherchez ou ajoutez un élément..."
            value={query}
